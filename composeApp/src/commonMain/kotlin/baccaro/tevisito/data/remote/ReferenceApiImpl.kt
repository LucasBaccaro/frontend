...
class ReferenceApiImpl(private val client: HttpClient) : ReferenceApi {
    ...
    override suspend fun getLocations(): List<LocationDto> {
        ...
        val apiResponse = Json { ignoreUnknownKeys = true }.decodeFromString<APIResponse<List<LocationDto>>>(body)
    ...
    override suspend fun getCategories(): List<CategoryDto> {
        ...
        val body = response.bodyAsText()
        val apiResponse = Json { ignoreUnknownKeys = true }.decodeFromString<APIResponse<List<CategoryDto>>>(body)
        return apiResponse.data ?: emptyList()
    }
} 